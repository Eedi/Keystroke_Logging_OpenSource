...
let myElement = document.getElementsByClassName('sc-dTOqkH eBehfv sc-gaREgu gOLGDh w-100')[0]; 

// Find the "Send" button to post keystroke data to the server end
let button = document.getElementsByClassName("sc-cWFgSD jHiKTB")[0];
...
if (keylog.TextContent.length >= 1 ){    // textchange     //activity
               
               let change = String(keylog.TextContent.slice(-1)).length - String(keylog.TextContent.slice(-2, -1)).length; 
                  ...
                  if (change == 0){
                          ...
                          if (start1 < end1 && start2 < end2 && Text1 == Text2){  // when move is detected
                              
                              if(start2 > start1 && end2 > end1){  //front move
                              ...
                              keylog.Activity.push(`Move From [${start1}, ${end1}] To [${start2}, ${end2}]`);
                              ActivityCancel.push(`Move From [${start1}, ${end1}] To [${start2}, ${end2}]`);
                              textNow = textNow.slice(0, start1) + textNow.slice(end1, end2) + movedText + textNow.slice(end2, textNow.length);
                              }else if (start2 < start1 && end2 < end1){  //back move 
                              ...
                              keylog.Activity.push(`Move From [${start1}, ${end1}] To [${start2}, ${end2}]`);
                              ActivityCancel.push(`Move From [${start1}, ${end1}] To [${start2}, ${end2}]`);
                              textNow = textNow.slice(0, start2) + movedText+ textNow.slice(start2, start1) + textNow.slice(end1, textNow.length);

                              }else if (start2 == start1 && end2 == end1){  // no move in the end
                                  textNow = textNow;
                          ...
                          }else if(start1 == end1 && start2 < end2 && String(textNow) != String(keylog.TextContent.slice(-1))){  // cancel this move
                              changeN = parseInt(ActivityCancel.length); 
                                  ...
                                  for (let i = 0; i <= changeN-1; i ++){
                                          ...
                                          if (activity.startsWith('Replace')){
                                              ...
                                              let substitute =  String(TextChangeCancel[num]).slice(5, middleindex);
                              ...
                              if (textNow == keylog.TextContent.slice(-1)){
                                 ...
                                 // start the else condition
                                  if(start < end){ // replace activity: replace n chracters with n new characters
                                    textNow = String(keylog.TextContent.slice(-2, -1)).slice(0, start) + 
                                      String(keylog.TextContent.slice(-1)).substr(start, end + change -start) + 
                                      String(keylog.TextContent.slice(-2, -1)).slice(end, String(keylog.TextContent.slice(-2, -1)).length);
                    
                                    replaced = String(keylog.TextContent.slice(-2, -1)).substr(start, end - start);
                                    substitute = String(keylog.TextContent.slice(-1)).substr(start, end + change -start);
                                    if(textNow == keylog.TextContent.slice(-1)){ 
                       ...
                       } if (start == end){ // irregular replacement 
                        AutoCorrectionDector(keylog);
              ...
              if (change == 1){
                  ...
                  let end = parseInt(endSelect.slice(-2, -1));
                   ...
                   textNow = textNow.slice(0, index-1) + String(keylog.TextContent.slice(-1))[index -1] + textNow.slice(index-1);
                   
                   if (textNow == keylog.TextContent.slice(-1)){ // just a common input 
  
                   keylog.TextChange.push(String(keylog.TextContent.slice(-1))[index -1]);
                   keylog.Activity.push('Input');
                   }else{  /// replace n characters with n+1 new characters

                      if(start < end){ /// regular paste activity
                            textNow = String(keylog.TextContent.slice(-2, -1)).slice(0, start) + 
                                      String(keylog.TextContent.slice(-1)).substr(start, end + change -start) + 
                                      String(keylog.TextContent.slice(-2, -1)).slice(end, String(keylog.TextContent.slice(-2, -1)).length);
               ...
               if (change > 1 ){
                  ...
                  let end = parseInt(endSelect.slice(-2, -1));
                   ...
                   let newlyAdded = String(keylog.TextContent.slice(-1)).slice(rangeStart, rangeEnd);
  
                   textNow = textNow.slice(0, rangeStart) + String(keylog.TextContent.slice(-1)).slice(rangeStart, rangeEnd) + textNow.slice(rangeStart);

                   if (textNow == keylog.TextContent.slice(-1)){ // Paste more than 1 character
                      keylog.TextChange.push(newlyAdded);
                      keylog.Activity.push('Paste');
   
                   }else{ // replace activity 
                          if(start < end){ // regular replace activity: replace n characters with n+m characters
                            textNow = String(keylog.TextContent.slice(-2, -1)).slice(0, start) + 
                                      String(keylog.TextContent.slice(-1)).substr(start, end + change -start) + 
                                      String(keylog.TextContent.slice(-2, -1)).slice(end, String(keylog.TextContent.slice(-2, -1)).length);
                    ...
                    if (start == end){ // irregular replace activity
                        AutoCorrectionDector(keylog);
                      }  
                  }
               }
               ...
               if (change == -1){
                  ...
                  if (textNow == keylog.TextContent.slice(-1)){ // backspace or delete to remove just one character
                  keylog.TextChange.push(deleted);
                  keylog.Activity.push('Remove/Cut');
                  }else{ 
  
                      if(start < end){ // regular replace activity: replace n characters with n-1 new characters
                         textNow = String(keylog.TextContent.slice(-2, -1)).slice(0, start) + 
                                      String(keylog.TextContent.slice(-1)).substr(start, end + change -start) + 
                                      String(keylog.TextContent.slice(-2, -1)).slice(end, String(keylog.TextContent.slice(-2, -1)).length);
               ...
               if (change < -1){
                  ...
                  if (textNow == keylog.TextContent.slice(-1)){ // delete more than 1 characters
                      keylog.TextChange.push(deleted);
                      keylog.Activity.push('Remove/Cut');
                  }else{
                      if(start < end){ // regular replace activity: replace n characters with n-m (m<n) new characters
                            textNow = String(keylog.TextContent.slice(-2, -1)).slice(0, start) + 
                                      String(keylog.TextContent.slice(-1)).substr(start, end + change -start) + 
                                      String(keylog.TextContent.slice(-2, -1)).slice(end, String(keylog.TextContent.slice(-2, -1)).length);
